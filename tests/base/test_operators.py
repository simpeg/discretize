...
class TestCurl2D(discretize.testing.OrderTest):
...
class TestCellGrad1D_InhomogeneousDirichlet(discretize.testing.OrderTest):
...
class TestCellGrad2D_Dirichlet(discretize.testing.OrderTest):
    name = "Cell Grad 2D - Dirichlet"
    meshTypes = ['uniformTensorMesh']
    meshDimension = 2
    meshSizes = [8, 16, 32, 64]
...
class TestCellGrad3D_Dirichlet(discretize.testing.OrderTest):
...
class TestCellGrad2D_Neumann(discretize.testing.OrderTest):
...
class TestCellGrad3D_Neumann(discretize.testing.OrderTest):
...
class TestFaceDiv2D(discretize.testing.OrderTest):
...
class TestAveraging2D(discretize.testing.OrderTest):
    name = "Averaging 2D"
    meshTypes = MESHTYPES
    meshDimension = 2
    meshSizes = [16, 32, 64]
