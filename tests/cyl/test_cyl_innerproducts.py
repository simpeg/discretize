...
class EdgeInnerProductFctsFacePropertiesIsotropic(object):
    ...
    def sol(self):
        ...
        int_r = sympy.lambdify(
            r, sympy.integrate(r * j_t**2 * tau_r, (z, 0, 1), (t, 0, 2 * np.pi)), "numpy"
        )
        
        int_z = sympy.lambdify(
            z, sympy.integrate(r * j_t**2 * tau_z, (r, 0, 1), (t, 0, 2 * np.pi)), "numpy"
        )
...
class TestCylInnerProducts_simple(unittest.TestCase):
    ...
    def test_FaceInnerProductFacePropertiesIsotropic(self):
        ...
        Mftau = self.mesh.get_face_inner_product_face_properties(tau)
    ...
    def test_EdgeInnerProductFacePropertiesIsotropic(self):
        ...
        tau, jv = fcts.vectors(self.mesh)
        Metau = self.mesh.get_edge_inner_product_face_properties(tau)
...
class TestCylFaceInnerProductsFaceProperties_Order(tests.OrderTest):
    ...
    def getError(self):
        ...
        Mtau = self.M.get_face_inner_product_face_properties(tau)
...
class TestCylEdgeInnerProductsFaceProperties_Order(tests.OrderTest):
    ...
    def getError(self):
        ...
        tau, jv = fct.vectors(self.M)
        Mtau = self.M.get_edge_inner_product_face_properties(tau)
        return float(fct.sol()) - jv.T.dot(Mtau.dot(jv))
...
class TestCylInnerProductsFaceProperties_Deriv(unittest.TestCase):
    ...
    def test_FaceInnerProductIsotropicDeriv(self):
        def fun(x):
            MfTau = self.mesh.get_face_inner_product_face_properties(x)
            MfTauDeriv = self.mesh.get_face_inner_product_face_properties_deriv(self.x0)
    ...
    def test_FaceInnerProductIsotropicDerivInvProp(self):
        def fun(x):
            MfTau = self.mesh.get_face_inner_product_face_properties(x, invert_model=True)
            MfTauDeriv = self.mesh.get_face_inner_product_face_properties_deriv(
                self.x0, invert_model=True
            )
            return MfTau * self.face_vec, MfTauDeriv(self.face_vec)
    ...
    def test_FaceInnerProductIsotropicDerivInvMat(self):
        def fun(x):
            MfTau = self.mesh.get_face_inner_product_face_properties(x, invert_matrix=True)
            MfTauDeriv = self.mesh.get_face_inner_product_face_properties_deriv(
                self.x0, invert_matrix=True
            )
            return MfTau * self.face_vec, MfTauDeriv(self.face_vec)
    ...
    def test_EdgeInnerProductIsotropicDeriv(self):
        def fun(x):
            MeTau = self.mesh.get_edge_inner_product_face_properties(x)
            MeTauDeriv = self.mesh.get_edge_inner_product_face_properties_deriv(self.x0)
    ...
    def test_EdgeInnerProductIsotropicDerivInvProp(self):
        def fun(x):
            MeTau = self.mesh.get_edge_inner_product_face_properties(x, invert_model=True)
            MeTauDeriv = self.mesh.get_edge_inner_product_face_properties_deriv(
                self.x0, invert_model=True
            )
            return MeTau * self.edge_vec, MeTauDeriv(self.edge_vec)
    ...
    def test_EdgeInnerProductIsotropicDerivInvMat(self):
        def fun(x):
            MeTau = self.mesh.get_edge_inner_product_face_properties(x, invert_matrix=True)
            MeTauDeriv = self.mesh.get_edge_inner_product_face_properties_deriv(
                self.x0, invert_matrix=True
            )
            return MeTau * self.edge_vec, MeTauDeriv(self.edge_vec)
