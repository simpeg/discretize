...
This patch contains a bug-fix related to plotting on a :class:`discretize.TreeMesh`.
It also contains a few additions which caused a minor functionality change in
:func:`discretize.utils.refine_tree_xyz`.

The first convenience is the addition of a `slice_loc` parameter to the
:func:`discretize.mixins.mpl_mod.InterfaceMPL.plot_slice`. This allows for slightly more
...
refine all cells that intersect a box (or list of boxes) with :func:`discretize.TreeMesh.refine_box`,
and similarly we can refine all cells that intersect a ball (or list of balls) with
:func:`discretize.TreeMesh.refine_ball`.
